...
import lexical_analysis.Current;
import lexical_analysis.LexicalAnalysis;
...
//    }
    ...
    private static Result declaration() {
        type();
        identList();
        token_consume(Token.SEMICOLON);
    }
    ...
    //arrumar reuslt
    private static Result restoAtrib() {
        Result r = new Result();
    ...
    private static Result and() {
        boolean flag = not();
        boolean flagAux = restoAnd();
        return (flag && flagAux);
    }

    private static Result restoAnd() {
    ...
    private static Result rel() {
        boolean flag = add();
        boolean flagAux = restoRel();
        return (flag && flagAux);
    }

    private static Result restoRel() {
        switch (actualToken.getToken()) {
            case EQUAL:
    ...
    private static Result restoAdd() {
        switch (actualToken.getToken()) {
            case SUM:
    ...
    private static Result mult() {
        boolean flag = uno();
        boolean flagAux = restoMult();
        return (flag && flagAux);
    }

    private static Result restoMult() {
        switch (actualToken.getToken()) {
            case MULT:
